...
export const UploadRxModal: React.FC<UploadRxModalProps> = ({
  ...
  // Real file upload handler — calls Gemini OCR endpoint to auto-populate fields
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const validMime = ['image/jpeg', 'image/png', 'image/webp', 'application/pdf'].includes(mimeType)
          ? (mimeType as 'image/jpeg' | 'image/png' | 'image/webp' | 'application/pdf')
          : ('image/jpeg' as const);
        ...
        try {
          const res = await fetch('/api/ai/ocr', {
            ...
            body: JSON.stringify({ imageBase64: base64, mimeType: validMime }),
          });
          if (res.ok) {
            const data = (await res.json()) as { extracted: Record<string, string | number> };
            const ex = data.extracted;
            if (ex.medicationName) setMedicationName(String(ex.medicationName));
            if (ex.strength) setStrength(String(ex.strength));
            if (ex.doctorName) setDoctorName(String(ex.doctorName));
            if (ex.clinicName) setClinicName(String(ex.clinicName));
            if (ex.sig) setSig(String(ex.sig));
            if (ex.qtyPrescribed) setQty(String(ex.qtyPrescribed));
            if (ex.refillsTotal) setRefills(String(ex.refillsTotal));
          }
        } catch {
          // fallback: fields stay at current values
        }
        setIsScanning(false);
        setScanComplete(true);
      };
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5">
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-slate-200 hover:border-teal-600 rounded-xl p-6 text-center bg-slate-50/50 transition-colors">
              ...
              {/* Real file input */}
              <label className="mt-3 inline-block cursor-pointer px-3 py-1.5 text-[11px] font-semibold bg-teal-700 hover:bg-teal-800 text-white rounded-lg transition-colors">
                Browse File
                <input
                  type="file"
